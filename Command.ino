        ...
        if (parameter[2]==(char)0) {
          ...
          if (parameter[0]=='F') { // Fn: Debug
            ...
            switch (parameter[1]) {
              ...
              case 'C': cli(); temp=siderealInterval; sei(); sprintf(reply,"%ld",temp); quietReply=true; break;                            // (L.F.) Sidereal interval
              case 'D': cli(); temp=SiderealRate; sei(); sprintf(reply,"%ld",temp); quietReply=true; break;                                // (L.F.) Sidereal Rate
              default:  commandError=true;
            }
          } else
            commandError=true;
...
//   Z - Environment sensors (L.F. added)
//  
        if (command[0]=='Z') {

           if(command[1]=='T') {
//  :ZT0# - Asks temperatures
               if(command[2]=='0'){ //ordina la lettura dei sensori
                  Tempbuffer= readDS18();
                  sprintf(reply,"%d", NSENSORS);
		  quietReply=true;
               } else {

//  :ZTN# - Read temperature sensor #N
		   int indexTemp= command[2]-'0';
		   if((indexTemp>0)&&(indexTemp<=NSENSORS)) {
			sprintf(reply,"%.1f", Tempbuffer[indexTemp]);
			quietReply=true;
